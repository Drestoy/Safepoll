...
use OAuth2\ClientAssertionType\HttpBasic2;
    ...
    UserInfoControllerInterface
{
    ...
    public function getTokenController(RequestInterface $request)
    {
        if (is_null($this->tokenController)) {
            $this->tokenController = $this->createDefaultTokenController($request);
        }
    ...
    {
        $this->response = is_null($response) ? new Response() : $response;
        $this->getTokenController($request)->handleTokenRequest($request, $this->response);
    ...
    {
        $this->response = is_null($response) ? new Response() : $response;
        $value = $this->getTokenController($request)->grantAccessToken($request, $this->response);
    ...
    public function addGrantType(GrantTypeInterface $grantType, $key = null)
    {
        ...
        if (!is_null($this->tokenController)) {
            $this->getTokenController($request)->addGrantType($grantType);
        }
    }
    ...
    protected function createDefaultTokenController(RequestInterface $request)
    {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof ClientAssertionTypeInterface) {
					...
					if($request->request("refresh_token")){
						...
						$config = array_intersect_key($this->config, array_flip(explode(' ', 'allow_credentials_in_request_body allow_public_clients')));
						$this->clientAssertionType = new HttpBasic2($this->storages['client_credentials'], $config);
						break;
					}
